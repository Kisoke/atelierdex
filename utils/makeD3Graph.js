...
function ForceGraph({
  ...
  const zoom = d3.zoom().on("zoom", e => {
    networkGroup.attr("transform", (transform = e.transform));
    networkGroup.style("stroke-width", 3 / Math.sqrt(transform.k));
  });
...
export default (elementId, nodes, links, nodeSize = { w: 50, h: 50 }, nodeRepulsion = 400) => {
  ForceGraph({ nodes: nodes, links: links, elementId: elementId })
}
